# Script for KTH guest lecture in Feb 2019
...
plot(simRes$betaSample[,2],simRes$betaSample[,3], xlab= expression(beta[1]), ylab= expression(beta[2]))

gridLength = 1000
xGrid = seq(min(x),max(x), length = gridLength)
fits = matrix(NA,gridLength,nIter)
for (i in nIter){
  fits[,i] = simRes$betaSample[i,1] + simRes$betaSample[i,2]*xGrid + simRes$betaSample[i,3]*xGrid^2
}
meanFit = apply(fits, 1, mean)
stdFit = apply(fits, 1, sd)
...
axis(side = 2, at = seq(-200, 200, by = 100))
lines(xGrid, fit, col ="red", lwd = lwdThinner)
lines(xGrid, fit+1.96*stdFit, col ="blue", lwd = lwdThinner)
